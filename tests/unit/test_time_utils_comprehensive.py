...
class TestTimeUtilsComprehensive:
    ...
    def test_within_trading_window_holiday(self, mock_holidays):
        ...
        market_time = datetime(2024, 1, 15, 10, 30, 0, tzinfo=ZoneInfo('America/New_York'))
        result = within_trading_window(market_time)
    ...
    def test_within_trading_window_buffer_timing(self, mock_holidays):
        ...
        buffer_start = datetime(2024, 1, 15, 8, 30, 0, tzinfo=ZoneInfo('America/New_York'))
        assert within_trading_window(buffer_start) is True
        
        # Test just before buffer (8:29 AM EST)
        before_buffer = datetime(2024, 1, 15, 8, 29, 0, tzinfo=ZoneInfo('America/New_York'))
        assert within_trading_window(before_buffer) is False
        
        # Test exactly at buffer end (4:30 PM EST)
        buffer_end = datetime(2024, 1, 15, 16, 30, 0, tzinfo=ZoneInfo('America/New_York'))
        assert within_trading_window(buffer_end) is True
        
        # Test just after buffer (4:31 PM EST)
        after_buffer = datetime(2024, 1, 15, 16, 31, 0, tzinfo=ZoneInfo('America/New_York'))
    ...
    def test_session_bounds_with_custom_day(self):
        ...
        custom_day = datetime(2024, 1, 15, 14, 30, 0, tzinfo=ZoneInfo('America/New_York'))
    ...
    def test_session_bounds_with_none_day(self, mock_now_est):
        ...
        current_time = datetime(2024, 1, 15, 14, 30, 0, tzinfo=ZoneInfo('America/New_York'))
    ...
    def test_next_session_open_with_custom_after(self, mock_now_est):
        ...
        with patch('alphagen.core.time_utils.session_bounds') as mock_bounds:
            ...
            next_session_start = datetime(2024, 1, 16, 8, 30, 0, tzinfo=ZoneInfo('America/New_York'))
            next_session_end = datetime(2024, 1, 16, 16, 30, 0, tzinfo=ZoneInfo('America/New_York'))
        ...
        with patch('alphagen.core.time_utils.session_bounds') as mock_bounds:
            ...
            def mock_bounds_side_effect(probe_day):
                if probe_day.date() == datetime(2024, 1, 16).date():
                    return (datetime(2024, 1, 16, 8, 30, 0, tzinfo=ZoneInfo('America/New_York')),
                            datetime(2024, 1, 16, 16, 30, 0, tzinfo=ZoneInfo('America/New_York')))
                elif probe_day.date() == datetime(2024, 1, 17).date():
                    return (datetime(2024, 1, 17, 8, 30, 0, tzinfo=ZoneInfo('America/New_York')),
                            datetime(2024, 1, 17, 16, 30, 0, tzinfo=ZoneInfo('America/New_York')))
                else:
                    return (datetime(2024, 1, 18, 8, 30, 0, tzinfo=ZoneInfo('America/New_York')),
                            datetime(2024, 1, 18, 16, 30, 0, tzinfo=ZoneInfo('America/New_York')))
            
            mock_bounds.side_effect = mock_bounds_side_effect
            
            with patch('alphagen.core.time_utils.US_MARKET_HOLIDAYS') as mock_holidays:
                def mock_holiday_check(date):
                    return date in [datetime(2024, 1, 16).date(), datetime(2024, 1, 17).date()]
                ...
                assert result == datetime(2024, 1, 18, 8, 30, 0, tzinfo=ZoneInfo('America/New_York'))
    ...
    def test_next_session_open_same_day_future_time(self, mock_now_est):
        ...
        current_time = datetime(2024, 1, 15, 6, 0, 0, tzinfo=ZoneInfo('America/New_York'))  # Before market
        mock_now_est.return_value = current_time
        
        with patch('alphagen.core.time_utils.session_bounds') as mock_bounds:
            # Same day session that starts later
            same_day_start = datetime(2024, 1, 15, 8, 30, 0, tzinfo=ZoneInfo('America/New_York'))
            same_day_end = datetime(2024, 1, 15, 16, 30, 0, tzinfo=ZoneInfo('America/New_York'))
    ...
    def test_to_est_with_custom_timezone(self):
        ...
        pst_time = datetime(2024, 1, 15, 6, 30, 0, tzinfo=ZoneInfo('America/Los_Angeles'))
    ...
    def test_session_bounds_different_days(self):
        ...
        test_dates = [
            datetime(2024, 1, 1, 12, 0, 0, tzinfo=ZoneInfo('America/New_York')),  # New Year's Day
            datetime(2024, 7, 4, 12, 0, 0, tzinfo=ZoneInfo('America/New_York')),  # July 4th
            datetime(2024, 12, 25, 12, 0, 0, tzinfo=ZoneInfo('America/New_York')),  # Christmas
        ]
    ...
    def test_now_est_timezone_consistency_multiple_calls(self):
        ...
        time_diffs = [abs((results[i] - results[i-1]).total_seconds()) for i in range(1, len(results))]
