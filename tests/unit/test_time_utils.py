...
class TestTimeUtils:
    ...
    def test_within_trading_window_with_custom_moment(self):
        ...
        custom_moment = datetime(2024, 1, 15, 14, 0, 0, tzinfo=EST)
    ...
    def test_session_bounds_returns_correct_bounds(self):
        ...
        expected_start = datetime.combine(
            datetime(2024, 1, 15).date(), MARKET_OPEN, tzinfo=EST
        ) - SESSION_BUFFER
        expected_end = datetime.combine(
            datetime(2024, 1, 15).date(), MARKET_CLOSE, tzinfo=EST
        ) + SESSION_BUFFER
    ...
    def test_session_bounds_with_custom_day(self):
        ...
        expected_start = datetime.combine(
            datetime(2024, 1, 16).date(), MARKET_OPEN, tzinfo=EST
        ) - SESSION_BUFFER
        expected_end = datetime.combine(
            datetime(2024, 1, 16).date(), MARKET_CLOSE, tzinfo=EST
        ) + SESSION_BUFFER
        
        assert start == expected_start
        assert end == expected_end

    @freeze_time("2024-01-15 22:00:00")  # After market close
    def test_next_session_open_skips_holidays(self):
        """Test next_session_open skips market holidays."""
        result = next_session_open()
        
        # Should be next trading day (Tuesday)
        assert result.date() == datetime(2024, 1, 16).date()
        assert result.hour == MARKET_OPEN.hour
        assert result.minute == MARKET_OPEN.minute
        assert result.tzinfo == EST
    ...
    def test_next_session_open_from_holiday(self):
        ...
        # Should skip the holiday and return next trading day
    ...
    def test_next_session_open_with_custom_after(self):
        ...
        custom_after = datetime(2024, 1, 15, 14, 0, 0, tzinfo=EST)
    ...
    def test_us_market_holidays_excludes_regular_weekdays(self):
        ...
        # Test regular Monday
        assert datetime(2024, 1, 15).date() not in US_MARKET_HOLIDAYS
        # Test regular Tuesday
