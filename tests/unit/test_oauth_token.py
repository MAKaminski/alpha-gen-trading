...
class TestOAuthTokenHandling:
    ...
    def oauth_client(self, mock_client):
        ...
        with patch('alphagen.schwab_oauth_client.client_from_token_file') as mock_token_file:
    ...
    @pytest.mark.asyncio
    async def test_fetch_equity_quote_with_token_refresh(self, oauth_client, mock_client):
        """Test equity quote fetching with token refresh."""
        # Mock successful token refresh and quote response
        mock_client.ensure_valid_access_token.return_value = None
        mock_client.get_quote = MagicMock(return_value={
            'QQQ': {
                'quote': {
                    'lastPrice': 400.0,
                    'bidPrice': 399.5,
    ...
    @pytest.mark.asyncio
    async def test_fetch_equity_quote_token_refresh_failure(self, oauth_client, mock_client):
        """Test equity quote fetching when token refresh fails."""
        # Mock token refresh failure
        mock_client.ensure_valid_access_token.side_effect = Exception("Token refresh failed")
    ...
    @pytest.mark.asyncio
    async def test_fetch_equity_quote_invalid_token_error(self, oauth_client, mock_client):
