...
class SchwabOAuthClient:
    ...
    async def fetch_positions(self) -> list[PositionSnapshot]:
        ...
        if not self._client:
            self._logger.error(
                "no_client_available",
                msg="OAuth2 client not initialized - cannot fetch real data",
            )
        ...
        try:
            ...
            if isinstance(account_info, dict) and "securitiesAccount" in account_info:
                ...
                self._logger.warning(
                    "unexpected_account_info_format",
                    account_info_type=type(account_info).__name__,
                )
    ...
    async def submit_order(self, intent: TradeIntent) -> TradeExecution:
        ...
        if not self._client:
            self._logger.warning(
                "no_client_available", msg="OAuth2 client not initialized"
            )
    ...
    async def fetch_equity_quote(self, symbol: str) -> EquityTick | None:
        ...
        try:
            ...
            if not await self._refresh_token_if_needed():
                self._logger.error(
                    "token_refresh_failed", msg="Cannot refresh OAuth token"
                )
                return None
        ...
        try:
            ...
            # Debug logging
            self._logger.debug(
                "schwab_quote_response",
                symbol=symbol,
                response_type=type(quote_data).__name__,
            )
            ...
            self._logger.warning(
                "no_quotes_found",
                symbol=symbol,
                response_keys=list(quote_data.keys())
                if isinstance(quote_data, dict)
                else "not_dict",
            )
            return None
            ...
            if "token_invalid" in error_msg or "InvalidTokenError" in error_msg:
                ...
                self._logger.error(
                    "equity_quote_fetch_failed", symbol=symbol, error=error_msg
                )
            return None
    ...
    async def fetch_option_quote(self, option_symbol: str) -> OptionQuote | None:
        ...
        try:
                            ...
                            if contract["symbol"] == option_symbol:
                                return OptionQuote(
                                    ...
                                    expiry=datetime.fromisoformat(
                                        contract["expirationDate"].replace(
                                            "Z", "+00:00"
                                        )
                                    ),
                                    as_of=to_est(datetime.now(timezone.utc)),
            ...
            self._logger.warning(
                "fetch_option_quote_error", symbol=option_symbol, error=str(e)
            )
            return None
