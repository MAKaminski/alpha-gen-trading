...
class SimpleGUChart:
    ...
    def __init__(self, parent_frame: tk.Widget, max_points: int = 100):
        ...
        # Initialize plot
        self.line_vwap, = self.ax.plot([], [], label="VWAP", color="#4caf50", linewidth=2)
        self.line_ma9, = self.ax.plot([], [], label="MA9", color="#2196f3", linewidth=2)
    ...
    def set_time_scale(self, scale: str) -> None:
        ...
        if scale in self.scale_configs:
            ...
            self.data_buffer = deque(maxlen=config["max_points"])
